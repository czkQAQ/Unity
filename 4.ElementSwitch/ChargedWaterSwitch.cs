...
public class ChargedWaterSwitch : BaseSwitch
{
    public ChargedWaterSwitch(ElementSwitch fsm) : base(fsm) { }
    ...
    {
        if (collision.tag == "Water" || collision.tag == "Trap_Lava" || collision.tag == "Trap_Gas")
        {
            //inSwitch = true;
            collision.gameObject.transform.parent.GetComponent<Element>().inSwitch = true;

            collision.gameObject.transform.parent.GetComponent<Element>()?.Transition(STATETYPE.CHARGEDWATER);
